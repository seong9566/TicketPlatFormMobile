...
## OVERVIEW
16 feature modules using feature-first Clean MVVM. Each feature is self-contained: data → domain → presentation.
...
| `chat` | Realtime SignalR chat + transaction state machine | Largest feature; SignalR datasource |
...
| `payment` | Toss Payments request → confirm | External SDK + confirm handshake |
...
| `search` | Ticket search by keyword | Lightweight, no domain layer |
| `sell` | 6-step ticket registration wizard | Most complex UI (ShellRoute) |
| `splash` | App init, auth gate, deep-link | Minimal — router redirect logic |
...
## CONVENTIONS
- Use-cases are single-method classes: one `call()` method, one responsibility.
- DTOs live in `data/dto/` (never in `domain/`); entities live in `domain/entities/` (never in `data/`).
- DTO → Entity conversion: implement `toEntity()` on DTO or repository impl — not in use-cases.
- Viewmodels extend `AsyncNotifier<T>` or `Notifier<T>` with `@riverpod` codegen annotation.
- `ui_models/` holds view-specific aggregations — not business entities, not raw DTOs.
- Providers in `providers/` wire up the DI graph (datasource → repo → usecase → viewmodel).
...
- Never use `context.read()` inside viewmodels — use the Riverpod `ref` passed to `AsyncNotifier`.
- Never skip the domain layer for "simple" features — at minimum keep the repository interface.
- Never place feature widgets in `lib/shared/widgets/` unless they are truly feature-agnostic.

## NOTES
- `search` feature currently has no `data/` or `domain/` layer — presentation only (local state).
- `splash` has no `data/` or `domain/` — just a viewmodel that checks auth state and redirects.
